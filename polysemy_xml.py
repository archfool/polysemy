...
class data_stream():
    ...
    def __call__(self):
        for i in range(math.ceil(self.n_corpus / self.batch_size)):
            ...
            if c_batch % 100 == 0:
                print(
                    "corpus: {}: {}".format(self.batch_size * c_batch, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
            # tensor_one_batch = infer_one_batch(one_batch_input, para, model_xml)
            yield word_ids, lengths, langs
...
if __name__=='__main__':
    ...
    data_generate = data_stream(params, batch_size)

    polysemt_xml_model = framework.framework(params=params, model=model, dataset=data_generate, optimizer=optimizer, loss_func=loss_func, USE_CUDA=USE_CUDA)
